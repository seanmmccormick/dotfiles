...
syn match scalaFqn "\<[._-zA-Z0-9,]*" contained nextgroup=scalaFqnSet
...
syn match scalaConstructor "\<[A-Z][_-zA-Z0-9]*\>" nextgroup=scalaConstructorSpecializer
syn region scalaConstructorSpecializer start="\[" end="\]" contained contains=scalaConstructorSpecializer

" method call
syn match scalaRoot "\<[a-zA-Z][_-zA-Z0-9]*\."me=e-1
syn match scalaMethodCall "\.[a-z][_-zA-Z0-9]*"ms=s+1

" type declarations in val/var/def
syn match scalaType ":\s*\(=>\s*\)\?[._-zA-Z0-9]\+\(\[[^]]*\]\+\)\?\(\s*\(<:\|>:\|#\|=>\)\s*[._-zA-Z0-9]\+\(\[[^]]*\]\+\)*\)*"ms=s+1
...
syn match scalaXmlComment "<-- --start 20110322 --end 20110324 --sampleRate 0.1\_[^>]*-->" contained
